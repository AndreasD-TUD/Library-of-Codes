...
Im Rahmen des Seminars befassen wir uns mit dem Datensatz des European Social Survey (ESS). Dieser Datensatz kann kostenlos von dem [ESS Data Portal](https://ess.sikt.no/en/?tab=overview) heruntergeladen werden. Dafür ist lediglich eine Anmeldung nötig, welche beispielsweise mit einem bestehenden Google-Account vollzogen werden kann. Die Daten finden sich alternativ auch im Opal-Kurs.
...
Im ersten Schritt müssen wir den Datensatz in dem Arbeitspfad ablegen, in welchem wir Arbeiten. Zur Erinnerung, den Arbeitspfad erhalten wir mit *`getwd()`* und setzten wir mit *`setwd()`*
